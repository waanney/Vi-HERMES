"""
Query RAG System: Query from Milvus (KB) and Neo4j (KG) and generate answer with GPT-4o
...
def setup_milvus_client(settings) -> MilvusClient:
    """Setup Milvus client with embedder."""
    try:
...
def build_enhanced_prompt(query: str, kb_results: List[dict], kg_results: List[dict]) -> str:
    """Build enhanced prompt with clear source attribution."""
    context_parts = []
    
    # Add KB results (Knowledge Base - Milvus)
    if kb_results:
        context_parts.append("=== NGUỒN TỪ KNOWLEDGE BASE (KB - Milvus) ===\n")
        for i, result in enumerate(kb_results, 1):
            source_info = f"[KB-{i}]"
            if result.get("doc_id"):
                source_info += f" Doc: {result['doc_id']}"
            if result.get("article_number"):
                source_info += f" | Điều: {result['article_number']}"
            if result.get("clause_number"):
                source_info += f" | Khoản: {result['clause_number']}"
            if result.get("title"):
                source_info += f" | Tiêu đề: {result['title']}"
            source_info += f" | Điểm số: {result.get('score', 0):.4f}"
            
            context_parts.append(f"{source_info}\n{result['text']}\n")
    
    # Add KG results (Knowledge Graph - Neo4j)
    if kg_results:
        context_parts.append("\n=== NGUỒN TỪ KNOWLEDGE GRAPH (KG - Neo4j) ===\n")
        for i, result in enumerate(kg_results, 1):
            source_info = f"[KG-{i}]"
            source_info += f" Loại: {result.get('label', 'Unknown')}"
            if result.get("doc_id"):
                source_info += f" | Doc: {result['doc_id']}"
            ...
            if result.get("number"):
                source_info += f" | Số: {result['number']}"
            if result.get("title"):
                source_info += f" | Tiêu đề: {result['title']}"
            source_info += f" | Điểm số: {result.get('score', 0):.4f}"
    ...
    prompt = f"""Bạn là chuyên gia pháp lý Việt Nam. Dưới đây là các nguồn thông tin từ Knowledge Base (KB) và Knowledge Graph (KG):
...
Yêu cầu:
1. Trả lời câu hỏi dựa trên các nguồn thông tin trên
2. Ghi rõ nguồn trích dẫn: [KB-X] cho Knowledge Base (Milvus) hoặc [KG-X] cho Knowledge Graph (Neo4j)
3. Nếu có thông tin từ cả KB và KG, hãy chỉ ra sự khác biệt hoặc bổ sung
4. Ghi rõ văn bản, điều, khoản, điểm (nếu có)
...
def query_rag_system(query: str, top_k: int = 5):
    ...
    try:
        neo4j = setup_neo4j_client(settings)
    ...
    # Query KB (Milvus)
    print("📊 Querying Knowledge Base (KB - Milvus)...")
    try:
        kb_results = query_milvus(milvus_client, query, top_k=top_k)
        print(f"✅ Found {len(kb_results)} results from KB")
        for i, result in enumerate(kb_results[:3], 1):
            print(f"   {i}. Doc: {result.get('doc_id')}, Score: {result.get('score', 0):.4f}")
    except Exception as e:
    ...
    # Query KG (Neo4j) using article_id and doc_id from KB results
    print("🔗 Querying Knowledge Graph (KG - Neo4j) using KB results...")
    try:
        kg_results = query_neo4j(neo4j, query, kb_results=kb_results, limit=top_k)
        print(f"✅ Found {len(kg_results)} results from KG")
        for i, result in enumerate(kg_results[:3], 1):
            label = result.get('label', 'Unknown')
            doc_id = result.get('doc_id', 'N/A')
            article_id = result.get('article_id', '')
            clause_id = result.get('clause_id', '')
            print(f"   {i}. {label}: {doc_id}", end="")
            if article_id:
                print(f" | Article: {article_id}", end="")
            if clause_id:
                print(f" | Clause: {clause_id}", end="")
    ...
    if not kb_results and not kg_results:
        print("❌ No results found from either KB or KG")
        return
    ...
    try:
        ...
        if kb_results:
            print("\nKnowledge Base (KB - Milvus):")
            for i, result in enumerate(kb_results, 1):
                print(f"  [KB-{i}] {result.get('doc_id')}")
                if result.get('article_number'):
                    print(f"       Điều: {result['article_number']}")
                if result.get('clause_number'):
                    print(f"       Khoản: {result['clause_number']}")
                print(f"       Score: {result.get('score', 0):.4f}")
        
        if kg_results:
            print("\nKnowledge Graph (KG - Neo4j):")
            for i, result in enumerate(kg_results, 1):
                print(f"  [KG-{i}] {result.get('label')}: {result.get('doc_id')}")
                if result.get('number'):
                    print(f"       Số: {result['number']}")
                print(f"       Score: {result.get('score', 0):.4f}")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # Interactive mode
        print("Query RAG System - Interactive Mode")
