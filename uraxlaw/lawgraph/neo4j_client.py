...
# Node labels (Giữ lại để tham khảo logic, dù model đã đổi sang str)
NODE_LABELS = [
    "Document", "Law", "Decree", "Circular", "Decision", "Resolution",
    "Article", "Clause", "Paragraph", "Term", "Concept",
    "CaseExample", "Agency", "ImportBatch", "ChangeLog",
]

# Relationship types
REL_TYPES = [
    "HAS_ARTICLE", "HAS_CLAUSE", "ISSUED_BY", "AMENDS", "REPEALS",
    "SUPPLEMENTS", "CITES", "DEFINED_IN", "MENTIONED_IN", "VERSION_OF",
    "HAS_KEYWORD", "APPLIES_TO", "REFERENCED_BY", "GUIDES",
    "REFERENCES", "BASED_ON", "INTERPRETS",
]
...
class Neo4jClient:
    ...
    def run_cypher(self, query: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        params = params or {}
        with self._driver.session() as session:
            # Thêm type: ignore để tắt lỗi Pylance về LiteralString
    ...
    def upsert_node(self, label: str, node_id: str, properties: Optional[Dict[str, Any]] = None) -> None:
        ...
        if label == "Document": id_prop = "doc_id"
        elif label == "Article": id_prop = "article_id"
        elif label == "Clause": id_prop = "clause_id"
        else: id_prop = "id"
    ...
    def upsert_edge(self, src_label: str, src_id: str, relation: str, tgt_label: str, tgt_id: str, properties: Optional[Dict[str, Any]] = None) -> None:
        properties = properties or {}
    ...
    def expand_related(self, node_id: Optional[str], max_hops: int = 1, limit: int = 50) -> List[Tuple[Node, Edge]]:
        ...
        for rec in recs:
            ...
            # Logic lấy ID đơn giản hóa cho demo
            tgt_node_id = rec.get("tgt_id") or rec.get("tgt_doc_id") or "unknown"
            src_node_id = rec.get("src_id") or rec.get("src_doc_id") or "unknown"
            
            # Tạo object Node/Edge (giờ đã chấp nhận str)
            ...
            edge = Edge(
                ...
                relation=rec.get("rel_type", "RELATED")
            )
    ...
    def _primary_label(self, labels: Iterable[str]) -> str:
        for lbl in labels:
            if lbl in NODE_LABELS: return lbl
        return "Document"
